...
(function () {
    ...
    function uid() {
        if (window.crypto && typeof window.crypto.getRandomValues === 'function') {
            ...
            return 'id-' + Array.prototype.map.call(b, function (x) {
    ...
    // merge an array of plain objects (left -> right), returns new object
    ...
    var Store = (function () {
        ...
        function upsert(entry) {
            ...
            if (idx >= 0) {
                updated = mergeArray([list[idx], entry, {updated_at: now}]);
                list[idx] = updated;
            } else {
                fresh = mergeArray([entry, {id: entry.id || uid(), created_at: now, updated_at: now}]);
                list.push(fresh);
            }
    ...
    function render() {
        ...
        list = Store.all().slice().sort(function (a, b) {
            var ta = new Date((a.date || '1970-01-01') + 'T' + (a.time || '00:00')).getTime();
            var tb = new Date((b.date || '1970-01-01') + 'T' + (b.time || '00:00')).getTime();
            return tb - ta;
        });
    ...
    function gatherForm() {
        ...
        var appetite = appetiteEl ? appetiteEl.value : '';
    ...
    function addHandlers() {
        ...
        // table actions
        (document.getElementById('entriesTable') || document).addEventListener('click', function (e) {
            var t = e.target;
            var act;
            var tr;
            var p;
